...
const fullReportPath =
  process.env.FULL_REPORT_FILE ||
  path.join(process.cwd(), 'test-results', 'full', 'playwright-report.json');
...
// ✅ Collect REAL failed spec filenames (unexpected only)
const failed = new Set();
...
fs.writeFileSync(riskDataPath, JSON.stringify(riskData, null, 2));
console.log('✅ failureHistory updated from REAL unexpected failures');
